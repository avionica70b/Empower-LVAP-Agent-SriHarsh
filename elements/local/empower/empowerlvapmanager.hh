...
public:
	EtherAddress _bssid;
	String _ssid;
	EmpowerNetwork(EtherAddress bssid, String ssid) : _bssid(bssid), _ssid(ssid)
	{
	}
	String unparse()
	{
...
public:
	...
	ResourceElement() : _iface_id(0), _hwaddr(EtherAddress()), _channel(1), _band(EMPOWER_BT_L20)
	{
	}

	ResourceElement(int iface_id, EtherAddress hwaddr, int channel, empower_bands_types band) : _iface_id(iface_id), _hwaddr(hwaddr), _channel(channel), _band(band)
	{
	}
...
public:
	...
	int handle_dscp_stats_request(Packet *, uint32_t);
	int handle_traffic_rules_response(Packet *, uint32_t);
	...
	void send_dscp_stats_response(String ssid, uint32_t xid);
	void send_traffic_rules_response(uint8_t dscp, empower_traffic_rule_match match);
...
private:
	...
	class TrafficRules *_tr;
	class DSCPStats *_dscpStats;
