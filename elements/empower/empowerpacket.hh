...
enum empower_packet_types
{
  ...
  // DSCP Stats
  // Request
  EMPOWER_PT_DSCP_STATS_REQUEST = 0x8D, // ac -> wtp
  // Response
  EMPOWER_PT_DSCP_STATS_RESPONSE = 0x8E, // wtp -> ac

  // Traffic Rules Response
  EMPOWER_PT_TRAFFIC_RULES_RESPONSE = 0x8F, // wtp -> ac
...
public:
  ...
  {
    memset(_ssid, 0, WIFI_NWID_MAXSIZE + 1);
    memcpy(_ssid, ssid.data(), ssid.length());
  }
...
/* DSCP stats request packet format */
struct empower_dscp_stats_request : public empower_header
{
private:
  char _ssid[WIFI_NWID_MAXSIZE + 1]; /* Null terminated SSID */
...
struct empower_dscp_map_entry
{
private:
  uint8_t _dscp_code;
  uint32_t _count;
  uint32_t _ave_packet_size;

public:
  void set_dscp_code(uint8_t code) { _dscp_code = code; }
  void set_count(uint32_t count) { _count = htonl(count); }
  void set_avg_packet_size(uint32_t ave_packet_size) { _ave_packet_size = htonl(ave_packet_size); }

} CLICK_SIZE_PACKED_ATTRIBUTE;

/* DSCP queue status packet format */
struct empower_dscp_stats_entry
{
private:
  // uint8_t _src_mac[6]; /* EtherAddress */
  // uint8_t _dst_mac[6]; /* EtherAddress */
  in_addr _src_ip;    /* IPAddress */
  in_addr _dst_ip;    /* IPAddress */
  uint16_t _src_port; /* Int */
  uint16_t _dst_port; /* Int */
  uint8_t _protocol;  /* Int */
  uint8_t _dscp;      /* Int */

public:
  // void set_src_mac(EtherAddress src_mac) { memcpy(_src_mac, src_mac.data(), 6); }
  // void set_dst_mac(EtherAddress dst_mac) { memcpy(_dst_mac, dst_mac.data(), 6); }
  void set_src_ip(in_addr src_ip) {_src_ip = src_ip; }
  void set_dst_ip(in_addr dst_ip) { _dst_ip = dst_ip; }
  void set_src_port(uint16_t src_port) { _src_port = htons(src_port); }
  void set_dst_port(uint16_t dst_port) { _dst_port = htons(dst_port); }
  void set_protocol(uint8_t protocol) { _protocol = protocol; }
  void set_dscp(uint8_t dscp) { _dscp = dscp; }

} CLICK_SIZE_PACKED_ATTRIBUTE;

/* DSCP stats response packet format */
struct empower_dscp_stats_response : public empower_header
{
private:
  char _ssid[WIFI_NWID_MAXSIZE + 1]; /* Null terminated SSID */
  uint16_t _nb_entries;              /* Int */
  uint8_t dscp_map_count;            /* Int */
public:
  void set_ssid(String ssid)
  {
    memset(_ssid, 0, WIFI_NWID_MAXSIZE + 1);
  ...
  void set_nb_entries(uint16_t nb_entries) { _nb_entries = htons(nb_entries); }
  void set_dscp_map_count(uint8_t count) { dscp_map_count = count; }
} CLICK_SIZE_PACKED_ATTRIBUTE;
...
/* Traffic Rule Match */
/* IMPORTANT - Any change here should be copied to trafficrules.hh */
...
public:
  ...
  in_addr get_src_ip() const { return _src_ip; }
  in_addr get_dst_ip() const { return _dst_ip; }
  uint16_t get_src_port() const { return _src_port; }
  uint16_t get_dst_port() const { return _dst_port; }
  uint8_t get_protocol() const { return _protocol; }
  uint8_t get_dscp() const { return _dscp; }

  void set_src_ip(in_addr src_ip) { this->_src_ip = src_ip; }
  void set_dst_ip(in_addr dst_ip) { this->_dst_ip = dst_ip; }
  void set_src_port(uint16_t src_port) { this->_src_port = src_port; }
  void set_dst_port(uint16_t dst_port) { this->_dst_port = dst_port; }
  void set_protocol(uint8_t protocol) { this->_protocol = protocol; }
  void set_dscp(uint8_t dscp) { this->_dscp = dscp; }

  bool operator==(const empower_traffic_rule_match &o) const
  {
    const char *IP_MATCH = "0.0.0.0";
    const uint16_t PORT_MATCH = 0;
    const uint8_t PROTOCOL_MATCH = 255;
    const uint8_t DSCP_MATCH = 255;
    return (inet_ntoa(this->_src_ip) == inet_ntoa(o.get_src_ip()) || inet_ntoa(o.get_src_ip()) == IP_MATCH) &&
           // return  this->_src_ip == o.get_src_ip() &&
           (inet_ntoa(this->_dst_ip) == inet_ntoa(o.get_dst_ip()) || inet_ntoa(o.get_dst_ip()) == IP_MATCH) &&
           (this->_src_port == o.get_src_port() || o.get_src_port() == PORT_MATCH) &&
           (this->_dst_port == o.get_dst_port() || o.get_dst_port() == PORT_MATCH) &&
           (this->_protocol == o.get_protocol() || o.get_protocol() == PROTOCOL_MATCH) &&
           (this->_dscp == o.get_dscp() || o.get_dscp() == DSCP_MATCH);
  }

  bool operator<(const empower_traffic_rule_match &o) const
  {
    return inet_ntoa(this->_src_ip) < inet_ntoa(o.get_src_ip()) ||
           inet_ntoa(this->_dst_ip) < inet_ntoa(o.get_dst_ip()) ||
           this->_src_port < o.get_src_port() ||
           this->_dst_port < o.get_dst_port() ||
           this->_protocol < o.get_protocol() ||
           this->_dscp < o.get_dscp();
  }
  ...
  // long unsigned int
  // This part has been inspired from hashcode.cc in include/click
  hashcode_t hashcode() const
  {
    IPAddress src_ip = IPAddress(this->_src_ip);
    IPAddress dst_ip = IPAddress(this->_dst_ip);
    return src_ip.hashcode() +
           dst_ip.hashcode() +
           this->_src_port +
           this->_dst_port +
           this->_protocol +
  ...
  {
    click_chatter("\n");
    click_chatter("Source IP: %s", inet_ntoa(_src_ip));
    click_chatter("Desctination Ip: %s", inet_ntoa(_dst_ip));
    click_chatter("Source Port: %d", _src_port);
    click_chatter("Destination Port: %d", _dst_port);
    click_chatter("Protocol: %d", _protocol);
    click_chatter("DSCP: %d", _dscp);
    click_chatter("\n");
  }
} CLICK_SIZE_PACKED_ATTRIBUTE;

/* Traffic Rule Response  */
struct empower_traffic_rules_response : public empower_header
{
private:
  uint8_t dscp;
...
public:
  uint8_t get_dscp() { return dscp; }
  empower_traffic_rule_match get_match() { return match; }
} CLICK_SIZE_PACKED_ATTRIBUTE;
